...
def transform():
    ...
    for path in os.listdir(dir_path):
        ...
        for num in range(1, count+1):
            branch_data = file_handling.get_data(num)
            
            if branch_data is not None:    # error handling
                for order in branch_data: # orders is the individual orders (dict{name: Michael, orders: chocolate})

                    purchase_string = order["orders"]
                    
                    items_list = purchase_string.split(", ")

                    split_name_and_price(items_list)
...
def split_name_and_price(items_list: list[str]): # function to strip the price from the name + falvour
    ...
    print(price_each)
    print(guids)
    #return prod_names, price_each, guids
